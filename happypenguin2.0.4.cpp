...
const float CROSSOVERS_PER_GAMETIC_CHROMOSOME = 0.78;
const int MIN_QUALITY_THRESHOLD = 14;
...
// version 2.0.2. low quality = 2 SNPs, medium quality = 3 SNPs, high quality = 4 SNPs, add quality in info file
// version 2.0.2. change data structure of SNPs
// version 2.0.3. improve output of crossover info. Estimation of crossover rate and comparison to baseline.
// version 2.0.4. (in progress) filters out low quality bases prior to forming genotype matrix

vector<tuple<unsigned int, unsigned int, char, char>>* SNP_coordinates;
...
unsigned long long total_read_bp = 0;

vector<tuple<long long,string>> recombinantinfo_vector[NUM_RECOMBINANT_CATEGORIES];

vector<unsigned long> genome_sizes = {0, 0};
vector<int> chromosome_sizes[2];
...
static string read_SNP_symbols[]      = {"1","2","O","X","-","L"};
// static string consensus_SNP_symbols[] = {"1","2","O","X","-","!","?","$"};

static char read_to_concensus[6][6] = {{'1','!','?','x','-','L'},
                                        {'!','2','?','x','-','L'},
                                        {'?','?','O','$','-','L'},
                                        {'x','x','$','X','-','L'},
                                        {'-','-','-','-','-','-'},
...
bool binarySearchTupleComparator(tuple<unsigned int, unsigned int, char, char> t1, tuple<unsigned int, unsigned int, char, char> t2) {
...
string reconstructAlignedSequence(stringstream& read, int& len, unsigned int& loc, string& chrom_name, bool& dir) {
    ...
    read >> num;
    read >> seq;       // field 10
    read >> base_qualities; // field 11

    dir = flag & 16; // check if 16 present in flag, indicating aligned reverse direction

    // base-quality filtering
    string filtered_seq = seq;
    int base_qual;
    for(int i = 0; i < int(seq.length()); i++) {
        base_qual = int(base_qualities[i]) - 33;
        if(base_qual <= MIN_QUALITY_THRESHOLD) { // non filtered starts with ASCII 0,1,2,3,4..., p-error > 0.0316227766017 bases are filtered out (replaced with N)
            filtered_seq[i] = 'N';
        }
    }

    // cout << "Before filtering:" << seq << endl;
    // cout << "Base qualities  :" << base_qualities << endl;
    // cout << "After filtering :" << filtered_seq << endl;
    ...
    if(!(cigar.str() == "*")) {
        while (cigar) {
            ...
            if (token == 'M' || token == '=' || token == 'X') {
                align_seq += filtered_seq.substr(seq_pos, val);
                seq_pos += val;
            } 
            else if (token == 'I') {
...
void analyzeReads() {
    ...
    while(sam_files[0]) {
        ...
        string line;
        int align_len[2];
        unsigned int align_loc[2];
        unsigned int align_end_loc[2];
        int align_chrom[2];
        ...
        auto it = lower_bound(SNP_coordinates[align_chrom[0]].begin(), SNP_coordinates[align_chrom[0]].end(), make_tuple(align_loc[0], 0, '-', '-'), binarySearchTupleComparator);
        while((get<0>(*it) < align_end_loc[0]) && (it != SNP_coordinates[align_chrom[0]].end())) {

            unsigned int pos1 = get<0>(*it);
            unsigned int pos2 = get<1>(*it);
            char nuc1 = get<2>(*it);
            char nuc2 = get<3>(*it);

            unsigned int relative_variant_pos[2];
            relative_variant_pos[0] = pos1 - align_loc[0];
            relative_variant_pos[1] = pos2 - align_loc[1];
            ...
            variantNucleotides[0].push_back(nuc1);
            variantNucleotides[1].push_back(nuc2);
        ...
        for(int n = 0; n < numSNPs; n++) {
            for(int i = 0; i < 2; i++) {
                if (i > 0 && (variantLocations[i][n] < 0 || variantLocations[i][n] > align_len[i])) {
                    ...
                    else if(readNucleotides[i][n] == 'N') readSNPs[i][n] = 5;
        ...
        if(numGTswitches == 0) {
            // 1. Count Errors
            // TO IMPLEMENT
            totalErrors += numIgnoredSNPs;
            totalErrorBP += numSNPs;
        ...
        int quality = breakpoint - 1;
        if(numUsefulSNPs - quality < quality) {
            quality = numUsefulSNPs - quality;
        }
        ...
        // "name\tchromosome\tcoordinate1\tcoordinate2\tfirstSNP\tquality\tnumUsefulSNPs\tnumIgnoredSNPs\talignDir(A:B)\tnumAlternateAlignments(A:B)" << endl;
        // "name\tcrossover\tchromosome\tmapcoordgenome1\tmapcoordgenome2\tfirstSNP\tquality\tnumUsefulSNPs\tnumIgnoredSNPs\talignDir(A:B)\tfractionBreakpoint"

        float relative_breakpoint = (float) (breakpoint - 1) / (float) numUsefulSNPs;

        stringstream rinfo;
        string left_bp = to_string(variantLocations[0][breakpoint - 2] + align_loc[0]);
        string right_bp = to_string(variantLocations[0][breakpoint - 1] + align_loc[0]);
        int padding = 20 - (left_bp.length() + right_bp.length());
        // string crossover = "chr" << (align_chrom[0] + 1) << ':' << setw(10) << setfill('0') << variantLocations[0][breakpoint] << '-' << setw(10) << setfill('0') << variantLocations[0][breakpoint + 1]
        rinfo << current_read_name << '\t' << "chr" << (align_chrom[0] + 1) << ':' << variantLocations[0][breakpoint - 2] + align_loc[0] << '-' << variantLocations[0][breakpoint - 1] + align_loc[0] << string(padding, ' ')
         << '\t' << (align_chrom[0] + 1) << '\t' << setw(10) << align_loc[0] << '\t' << setw(10) << align_loc[1] << '\t' << firstSNP << '\t' << quality << '\t' << numUsefulSNPs << '\t' << numIgnoredSNPs << '\t' << ALIGN_DIRECTIONS[align_dir[0]] << ':' << ALIGN_DIRECTIONS[align_dir[1]] << '\t' << setprecision(3) << relative_breakpoint;
        ...
        categorizeRead(2 + category, current_read_name, SAM_line[0], SAM_line[1], textoutput, silenced, align_len[1]);

        long long coordinatevalue = genome_sizes[0] * align_chrom[0] + align_loc[0];
        recombinantinfo_vector[category].push_back(make_tuple(coordinatevalue,rinfo.str()));
...
void printCategorizationResults(double time) {
    ...
    float avg_len = total_read_bp / total_reads;

    // avg_len = 10634;

    // total_reads = 7245248;

    float effective_genome_size = ((genome_sizes[0] + genome_sizes[1]) / 2);

    float estimated_crossover_molecules = (avg_len * total_reads * num_chromosomes * CROSSOVERS_PER_GAMETIC_CHROMOSOME / (effective_genome_size) );
    
    int num_crossovers = num_reads_in_category[4];
    float crossover_score = num_crossovers / estimated_crossover_molecules;

    summary << endl << "(Average read length " << setw(10) << setprecision(3) << avg_len << "bp)" << endl;
    summary << "Expected number of crossover molecules (wild type): " << setw(12) << setprecision(3) << estimated_crossover_molecules << endl;
    summary << "Crossover score: " << setprecision(3) << crossover_score << "X" << endl;
...
void makeDirectories(string base_dir) {
    ...
    for(int i = 0; i < NUM_RECOMBINANT_CATEGORIES; i++) {
        ...
        recombinantinfo_files[i] << "name\tcrossover\tchromosome\tmapcoordgenome1\tmapcoordgenome2\tfirstSNP\tquality\tnumUsefulSNPs\tnumIgnoredSNPs\talignDir(A:B)\tfractionBreakpoint" << endl;
    }
...
void createSNPfile(string polymorphism_file) {
    ...
    while(p_file >> chrName[0]) {
        ...
        if(parent_id.substr(0, 3) == "SYN" && type == "SNP") {
            ...
            // write to SNP file
            p_SNP_file << chrNum[0] + 1 << '\t' << chrLoc[0] << '\t' << chrLoc[1] << '\t' << seq[0] << '\t' << seq[1] << '\n';

            // write to both VCF files
            for(int i = 0; i < 2; i++) {
                // CHROM	POS	ID	REF	ALT	QUAL	FILTER	INFO
                p_VCF_files[i] << (chromosome_names[i][chrNum[i]] + "\t" + to_string(chrLoc[i]) + "\t" + unique_id + "\t" + seq[i] + "\t" + seq[i ^ true] + "\t.\tPASS\t." ) << '\n';
            }
...
void readSNPs(string polymorphism_file) {
    ...
    unsigned int chr1loc, chr2loc;
    char nuc1, nuc2;
    int chromosome;
    int count = 0;
    //
    while(p_SNP_file >> chromosome >> chr1loc >> chr2loc >> nuc1 >> nuc2) {
        SNP_coordinates[chromosome - 1].push_back(make_tuple(chr1loc, chr2loc, nuc1, nuc2));
        count ++;
    }
}
...
void readChromosomes(string alignment_genome_Afile, string alignment_genome_Bfile) {
    ...
    for(int i = 0; i < 2; i++) {
        ...
        while(true) {
            ...
            if(str == "@SQ") { // genomic alignment reference sequence (i.e. chromosome)
                ...
                chromosome_sizes[i].push_back(len);
                genome_sizes[i] += len;
                n++;
    ...
    num_chromosomes = chromosome_sizes[1].size();

    for(int i = 0; i < 2; i++) {
        log_file << "[genome] " << i << " len: " << genome_sizes[i] << endl;
    }    

    cout << "[happypenguin] Allocating Memory for SNP locations and SNP nucleotides" << endl;

    SNP_coordinates = (vector<tuple<unsigned int, unsigned int, char, char>>*)  malloc(num_chromosomes * sizeof(vector<tuple<unsigned int, unsigned int, char, char>>));

    for(int i = 0; i < num_chromosomes; i++) {
        new (&SNP_coordinates[i]) vector<tuple<unsigned int, unsigned int, char, char>>();
    }
...
string getPath(const string& name) {
    ...
    FILE* pipe_stream = popen(("which " + name + " 2> /dev/null").c_str(), "r");

    while (fgets(buffer.data(), buffer.size(), pipe_stream) != NULL) {
        result += buffer.data(); // .data() returns pointer to string, handling conversions between c and c++ strings?
    }
    pclose(pipe_stream);
...
void generatePlots(string base_dir) {
    for(int i = 0; i < NUM_CATEGORIES + 1; i++) {
        readlength_files[i].close();
    }
    string python_script = getPath("generatePlots.py");
    string python_interpreter = getPath("python3");
    string command = python_interpreter + " " + python_script + " " + base_dir;
